...
- **Level backgrounds**: `INIT_LEVEL_BG` fills the interior per level. Levels 2+ reveal one of three rotating photo images (`OVERLAY_LEVEL2/4_BG`, `DECOMPRESS_LEVEL10_BG`, chosen by `LEVEL` mod 3). **Level 1** (`LEVEL == 1`, entered via `START_NEW_GAME` which now also calls `INIT_LEVEL_BG`) instead gets `DRAW_STARFIELD` — a sparse faint dot field (each interior cell picks one of 8 `STAR_PATTERNS` at random; dim grey dots over black) so it reads as a dark surface, not dead black. Starfield code/data live in the `$3880` gap.
